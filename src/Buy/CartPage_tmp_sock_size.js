...
class CartPage extends Component {
    ...
    checkoutSuccess(res) {
        ...
        if (res.data.result === Api.OK) {
            ...
            this.props.sockCart.map((item, i) => {
                items = [
                    ...items,
                    {
                        product: item.infoValue + ' ' + item.productName,
            ...
            // reset carts
            Storage.setPassCart( JSON.stringify([]) )
            Storage.setShoeCart( JSON.stringify([]) )
            Storage.setSockCart( JSON.stringify([]) )
            value = {
        ...
        this.showProgress()
            ...
            this.props.passCart.map((item, i) => {
                ...
                if (item.type == Api.SEASONPASS_TYPE) {
                    ...
                    passAvailableDate.setMonth( passAvailableDate.getMonth() + 6 )
    ...
    delete = (category, index) => {
        if (category == Configuration.CATEGORY_PASS) {
            ...
            var updatedCart = Util.removeItem(this.props.sockCart, action)
